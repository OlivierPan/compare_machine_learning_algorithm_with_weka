...
def bag_of_words(nom_fichier, mots_vides, characters=False):
    ...
    return Counter(
        l
        #for w in filter(None, re.split(r"[\s\.]+", text))#<----- Cette ligne est pour segmenter en token pour le français
        for w in filter(None, list(jieba.cut(text))) #<----- Cette ligne est pour segmenter en mot chinois
        if not w.isspace()
        for l in (w.lower(),)
